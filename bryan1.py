...
async def logUserBot():
    load_dotenv()
    api_id = int(27657418)
    api_hash = "c351490cd3083360fdcd180e41561543"
    phone_number = "51983468460"
    ...
    await client.send_message("@spmav1", f'<b>Bot encendido</b>', parse_mode="HTML")
    spammer_group = int("-4534678905")
    spammer_group2 = int("-4597564105")

    # Lista de IDs de grupos/canales a los que no quieres enviar mensajes
    excluded_group_ids = [-4534678905,-1002474821240,-4597564105,-1001907073788,-1001580673964,-1001737351681,-1001829996546,-1002018025154,-1001733869168,-1001748356782,-1001859082953]  # Reemplaza con los IDs de los grupos a excluir
    special_group_ids = [-1001617010310,-1002074331354,-1002221235561,-1001867739320,-1001789640951,-1001890531963,-1001983920380,-1001724620371]  # Reemplaza con los IDs de los grupos especiales
    ...
    while True:
        ...
        try:
            await client.send_message("@spmav1", f"<b>CANTIDAD DE MENSAJES CONSEGUIDOS PARA PUBLICAR</b> <code>{len(messages_list)-1}</code>", parse_mode="HTML")
        except:
            pass
                    ...
                    if i['group_id'] in special_group_ids:  # Compara por group_id en lugar de group_name
                        ...
                        for message_spam in special_messages_list:
                            ...
                            try:
                                ...
                                await client.send_message("@spmav1", f'<b>Error enviando mensajes a {i["group_id"]}</b> - <code>{i["group_name"]}<code>\nCausa:{error}', parse_mode="HTML")
                                resultado = False
                            if resultado:
                                await client.send_message("@spmav1", f'<b>Mensaje enviado a {i["group_id"]}</b> - <code>{i["group_name"]}</code>', parse_mode="HTML")  
                        ...
                        for message_spam in messages_list:
                            ...
                            if j == 4: break
            await client.send_message("@spmav1", f'<b>RONDA ACABADA</b>', parse_mode="HTML")
